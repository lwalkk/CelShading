...
void GPUProgram::validateProgram()

{
  ...
  if (length > 0) {
    std::cout << "Program " << program_id << " link log: " << buffer << std::endl;
    exit(1);
  }
...

{
  ...
  vsText = strdup(vsText);	// in case it's an immutable string
  ...
  fsText = strdup(fsText);	// in case it's an immutable string
...
void GPUProgram::initFromFile( const char *vsFile, const char *fsFile ) 
